...
def traj_segment_generator(pi, env, horizon, path, stochastic):
    t = 0
    ...
    while True:
        ...
        ob, rew, new, envinfo = env.step(ac)          ######### rewards to be collected
        ...
        file_name = "obs_{}.json".format(postfix)
        
        ob_list = ob.tolist()
        if time_steps < 100:
            collect_obs.append([time_steps, ob_list])
            with open(os.path.join(path, file_name), 'w+') as outfile_2:
                json.dump(collect_obs, outfile_2)
                
        #images[0].save('./video/tt-'+str(l)+'.gif', save_all=True, append_images=images[1:], optimize=False, duration=40, loop=0)
        ...
        if new:
            ...
            if not broke:
                ...
                all_eps_rets.append(cur_ep_ret)
        ...
        # log_eps_rets += cur_ep_ret
        if time_steps % log_freq == 0: 
            config_name = path
            log_avg_reward = (sum(all_eps_rets)/len(all_eps_rets))
            plot_data.append([time_steps, log_avg_reward])

            with open(os.path.join(config_name, 'train_ep_rets.json'), 'w+') as outfile:
                json.dump(plot_data, outfile)
            
            # savetxt(os.path.join(config_name, 'ob_data.csv'), ob, delimiter=',')
            # # with open(os.path.join(config_name, 'train_ep_obs.txt'), 'w+') as outfile_2:
            #     outfile_2.write(str(ob))
            

                #changed from ep_rets to log_eps_rets
...
def learn(env, policy_func, folder_path, *,
    ...
    # ----------------------------------------
    seg_gen = traj_segment_generator(pi, env, timesteps_per_batch, folder_path, stochastic=True)
    episodes_so_far = 0
    timesteps_so_far = 0
    iters_so_far = 0
    tstart = time.time()
