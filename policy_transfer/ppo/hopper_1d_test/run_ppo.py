...
def train(l_b4, t4, n_iterations, env_id, num_timesteps, seed, batch_size, clip, 
                schedule, mirror, warmstart, train_up, folder_path):
    from policy_transfer.ppo.hopper_2d_test import ppo_sgd
    ...
    extn_env = EnvWrapper(env, up_dim=[l_b4, t4])   #wrapped env

    timesteps = num_timesteps * n_iterations
    batch_size = 4000                                # changed to 4000 from 1024 to match that of HB-PPO
    ...
    pi, avg_training_reward = ppo_sgd.learn(extn_env, pol_func, folder_path,
            ...
            gamma=0.99, lam=0.95, schedule=schedule,
                        callback=callback, init_policy_params=warmstart_params) #optim_batchsize = 64
...
def main():
    ...
    # parser2.add_argument('--l_b3', help='length', type=float, default=None)
    parser2.add_argument('--l_b4', help='length', type=float, default=None)
    # parser2.add_argument('--t1', help='mass', type=float, default=None)
    # parser2.add_argument('--t2', help='mass', type=float, default=None)
    # parser2.add_argument('--t3', help='mass', type=float, default=None)
    parser2.add_argument('--t4', help='mass', type=float, default=None)
    ...
    parser2.add_argument('--config_name', help='setting logger path for UPN policy')
    


    args3 = parser2.parse_args()
    print("######### Length values for training #########", args3.l_b4)
    print("######### Size values for training #########", args3.t4)
    ...
    pi, avg_rew = train(l_b4, t4, n_iterations, args3.env, num_timesteps=int(args3.min_timesteps), 
    ...
    config_name = args3.config_name
    logger.configure(config_name, ['json','stdout'])

    config_name2 = args3.path        
    data = {'pi': logger.get_dir() + '/policy_params' + '.pkl', 'avg_rew': avg_rew}
    with open(os.path.join(config_name2, 'test.json'), 'w') as outfile:
