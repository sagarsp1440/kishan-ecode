...
'''
space = {
        'l_b4': hp.uniform('l_b4', 0.13, 0.39), 
        't4': hp.uniform('t4', 0.03, 0.09)
         }
...
def train_params(args):
    def _train_params(n_iterations, params, xml_path, folder_path, config_name):
        ...
        args2 = ["python", "policy_transfer/ppo/hopper_2d_test/run_ppo.py", "--l_b4", l_b4, "--t4", t4,  "--n_iter", n_iterations, "--warmstart", UPN_PI, "--env", args.env]
        args2 += ["--seed", 0, "--name", args.name, "--min_timesteps", args.min_timesteps]
...
def main():
    ...
    hb = Hyperband(get_params, train_params(args), xml_path, data_folder, args.seed, config_name)
