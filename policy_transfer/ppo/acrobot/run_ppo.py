...
def train(l1, l2, ml1, ml2, n_iterations, env_id, num_timesteps, seed, batch_size, clip, 
                schedule, mirror, warmstart, train_up, folder_path):
    from policy_transfer.ppo.acrobot import ppo_sgd
    sess = U.make_session(num_cpu=1)
    sess.__enter__()
    set_global_seeds(seed)

    env = gym.make(env_id, l1=l1, l2=l2, ml1=ml1, ml2=ml2)
    if train_up:
        if env.env.train_UP is not True:
            ...
            env.env.obs_dim = 6 + 4  # 6 because obs size is 6; +4 because 4 latent parameters
    ...
    extn_env = EnvWrapper(env, up_dim=[l1,l2,ml1,ml2])   #wrapped env
...
def main():
    ...
    parser2.add_argument('--l1', help='length', type=float, default=None)
    parser2.add_argument('--l2', help='length', type=float, default=None)
    parser2.add_argument('--ml1', help='mass', type=float, default=None)
    parser2.add_argument('--ml2', help='mass', type=float, default=None)
    ...
    print("######### Length values for training #########", args3.l1, args3.l2)
    print("######### Mass values for training #########", args3.ml1, args3.ml2)
    ...
    pi, avg_rew = train(l1, l2, ml1, ml2, n_iterations, args3.env, num_timesteps=int(args3.min_timesteps), 
