...
class Hyperband:
    ...
    def __init__(self, get_params_function, train_params_function, xml_path, data_folder_path, seed, val1, val2):
        ...
        self.max_episodes = 81  # maximum episodes per configuration/no. of configurations 
    ...
    def run(self, skip_last=0, dry_run=False):
        ...
        for s in range(self.s_max + 1):
            ...
            rng = np.random.RandomState(seed=s*10+self.seed)
            T = [self.get_params(rng) for i in range(num_config)]
            print("T", T)
            ...
            for i in range((s + 2) - int(skip_last)):  # changed from s + 1    #changed s+1 to s+2 - Kishan
                ...
                for t in T:
                    ...
                    result = self.train_params(n_iterations, t, self.xml_path, self.data_folder_path, self.val1, self.val2)
