...
space = {'l_th': hp.uniform('l_th', -0.34*0.5, -0.34*1.5), 'l_sh': hp.uniform('l_sh', -0.3*0.5, -0.3*1.5),
        ...
        #  'l_t_s': hp.uniform('l_t_s', 0.06*0.5, 0.06*1.5), 'l_s_s': hp.uniform('l_f_s', 0.049*0.5, 0.049*1.5), 
         'r_th': hp.uniform('r_th', -0.34*0.5, -0.34*1.5), 'r_sh': hp.uniform('r_sh', -0.3*0.5, -0.3*1.5), 
         'r_f_s': hp.uniform('r_f_s', 0.075*0.5, 0.075*1.5), 
        #  'r_t_s': hp.uniform('l_t_s', 0.06*0.5, 0.06*1.5), 'r_s_s': hp.uniform('l_f_s', 0.049*0.5, 0.049*1.5), #legs
...
def train_params(args):
    def _train_params(n_iterations, params, xml_path, folder_path):
        ...
        args2 = ["python", "policy_transfer/ppo/humanoid/run_ppo.py"]
        args2 += ["--l_th", l_th, "--l_sh", l_sh, "--l_f_s", l_f_s] 
        args2 += ["--r_th", r_th, "--r_sh", r_sh, "--r_f_s", r_f_s]
        # args2 += ["--l_t_s", l_t_s, "--l_s_s", l_s_s, "--r_t_s", r_t_s, "--r_s_s", r_s_s]
        ...
        plt.savefig(os.path.join(folder_path,'./graphs/graph_{}_{}.png'.format(l_th, l_th-np.random.random())))                  #location
...
def main():
    ...
    hb = Hyperband(get_params, train_params(args), xml_path, data_folder, args.seed, args.config_count)
