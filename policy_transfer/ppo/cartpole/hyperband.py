...
class Hyperband:
    ...
    def __init__(self, get_params_function, train_params_function, data_folder_path, seed):
        self.get_params = get_params_function
        self.train_params = train_params_function
        self.data_folder_path = data_folder_path
        self.seed = seed

        self.max_episodes = 27  # maximum episodes per configuration
    ...
    def run(self, skip_last=0, dry_run=False):
        ...
        for s in range(self.s_max + 1):
            ...
            T = [self.get_params(rng=rng) for i in range(num_config)]
            print("T", T)
            ...
            for i in range((s + 2) - int(skip_last)):  # changed from s + 1    #changed s+1 to s+2 - Kishan
                ...
                for t in T:
                    ...
                    result = self.train_params(n_iterations, t, self.data_folder_path)
        ...
        #         # select a number of the best configurations for the next loop
        #         # filter out early stops, if any
        #         result = self.eval_params(T, n_iterations)
        #         all_returns = []
        #         for i in range(len(result)):
        #             x = result[i]
        #             i_results = {'Return': x}
        #             assert (type(i_results) == dict)
        #             assert ('Return' in i_results)
        #             all_returns.append(i_results)
        #         print("Result", all_returns)  
        ...
        #             all_returns[i]['counter'] = self.counter
        #             #all_returns[i]['seconds'] = seconds
        #             all_returns[i]['params'] = T[i]
        #             all_returns[i]['iterations'] = n_iterations
        #             all_returns[i]['id'] = s
        #             self.results.append(all_returns)
        ...
        # #print("Self.results from hyperband ------", self.results)
