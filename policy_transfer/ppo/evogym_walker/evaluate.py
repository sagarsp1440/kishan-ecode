...
def main():
    ...
    env = gym.make("CartPole-v1", length = 1)
    ...
    for l in lengths:
        ...
        while not dones:
            ...
            obs, rewards, _ , info = extn_env.step(action)
            ...
            epi_count += 1
            if epi_count > 5*n_iterations:
                    dones = True

        #rl_return = np.array(rl_return)
        avg_rlreturn = np.average(rl_return, axis=0)
        print("Avg_rlreturn", avg_rlreturn, type(avg_rlreturn))
        avg_rlreturns.append(avg_rlreturn) 
