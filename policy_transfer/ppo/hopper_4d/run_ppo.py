...
def train(l_b1, l_b2, l_b3, l_b4, t1, t2, t3, t4, n_iterations, env_id, num_timesteps, seed, 
            batch_size, clip, schedule, mirror, warmstart, train_up, folder_path):
    from policy_transfer.ppo.hopper_8d import ppo_sgd
    ...
    if train_up:
        if env.env.train_UP is not True:
            ...
            env.env.obs_dim = 11 + 8  # 6 because obs size is 6; +4 because 4 latent parameters
    ...
    extn_env = EnvWrapper(env, up_dim=[l_b1, l_b2, l_b3, l_b4, t1, t2, t3, t4])   #wrapped env
    ...
    pi, avg_training_reward = ppo_sgd.learn(extn_env, pol_func, folder_path,
            ...
            gamma=0.99, lam=0.95, schedule=schedule,
                        callback=callback, init_policy_params=warmstart_params)
...
def main():
    ...
    print("######### Length values for training #########", args3.l_b1, args3.l_b2, args3.l_b3, args3.l_b4)
    print("######### Size values for training #########", args3.t1, args3.t2, args3.t3, args3.t4)
    ...
    pi, avg_rew = train(l_b1, l_b2, l_b3, l_b4, t1, t2, t3, t4, n_iterations, args3.env, num_timesteps=int(args3.min_timesteps), 
