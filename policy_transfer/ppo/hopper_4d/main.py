...
t1 = 0.05, t2 = 0.05, t3 = 0.04, t4 = 0.06 
'''
space = {'l_b1': hp.uniform('l_b1', 0.2, 0.6), 'l_b2': hp.uniform('l_b2', 0.225, 0.675), 
        'l_b3': hp.uniform('l_b3', 0.25, 0.75), 'l_b4': hp.uniform('l_b4', 0.19, 0.58), 
        't1': 0.05, 't2': 0.05, 
        't3': 0.04, 't4': 0.06
         }
...
def train_params(args):
    def _train_params(n_iterations, params, xml_path, folder_path):
        ...
        args2 = ["python", "policy_transfer/ppo/hopper_8d/run_ppo.py", "--l_b1", l_b1, "--l_b2", l_b2, "--l_b3", l_b3, "--l_b4", l_b4, "--t1", t1, "--t2", t2, "--t3", t3, "--t4", t4, "--n_iter", n_iterations, "--warmstart", UPN_PI, "--env", args.env]
        args2 += ["--seed", args.UPN_seed, "--name", args.name, "--min_timesteps", args.min_timesteps]
...
def main():
    ...
    parser.add_argument('--UPN_seed', help='RNG seed', type=int, default=2)
    ...
    hb = Hyperband(get_params, train_params(args), xml_path, data_folder, args.seed)
