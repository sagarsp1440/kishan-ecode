...
l_b1 = 0.4, l_b2 = 0.45, l_b3 = 0.5, l_b4 = 0.2   [[[[symetrical walker, for asym. change xml body tag names]]]]
'''
space = {'l_b1': hp.uniform('l_b1', 0.2, 0.6), 'l_b2': hp.uniform('l_b2', 0.225, 0.675), 
        'l_b3': hp.uniform('l_b3', 0.25, 0.75), 'l_b4': hp.uniform('l_b4', 0.1, 0.3)}
...
def train_params(args):
    def _train_params(n_iterations, params, xml_path, folder_path, val1, val2, val3, val4):
        ...
        args2 = ["python", "policy_transfer/ppo/walker_4d/run_ppo.py", "--l_b1", l_b1, "--l_b2", l_b2, "--l_b3", l_b3, "--l_b4", l_b4, "--t1", t1, "--t2", t2, "--t3", t3, "--t4", t4, "--n_iter", n_iterations, "--warmstart", "", "--env", args.env]
        ...
        plt.savefig(os.path.join(folder_path,'./graphs/graph_{}_{}.png'.format(l_b4, l_b4-np.random.random())))                  #location
...
def main():
    ...
    parser.add_argument("--val3")
    parser.add_argument("--val4")
    ...
    hb = Hyperband(get_params, train_params(args), xml_path, data_folder, args.seed, float(val1), float(val2), float(val3), float(val4))
