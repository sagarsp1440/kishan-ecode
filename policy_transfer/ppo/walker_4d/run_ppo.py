...
def train(l_b1, l_b2, l_b3, l_b4, t1, t2, t3, t4, n_iterations, env_id, num_timesteps, seed, 
            batch_size, clip, schedule, mirror, warmstart, train_up, folder_path, xml_path):
    from policy_transfer.ppo.walker_4d import ppo_sgd
    ...
    if train_up:
        if env.env.train_UP is not True:
            ...
            env.env.obs_dim = 17 + 4  # 6 because obs size is 17; +8 because 8 latent parameters
    ...
    extn_env = EnvWrapper(env, up_dim=[l_b1, l_b2, l_b3, l_b4])   #wrapped env
